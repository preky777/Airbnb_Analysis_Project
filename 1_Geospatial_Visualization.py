...
# Extract the review rating from the review_scores column
def extract_review_rating(review_scores):
    try:
        review_scores_dict = ast.literal_eval(review_scores)
        return review_scores_dict.get('review_scores_rating', None)
    except (SyntaxError, ValueError):
        return None

# Apply the function to extract review rating
df['review_rating'] = df['review_scores'].apply(extract_review_rating)
...
def main():
    ...
    for i in range(len(filtered_df)):
        if filtered_df['address'].iloc[i]:
            ...
            popup_text = f"Location: {loc}\nProperty Type: {filtered_df['property_type'].iloc[i]}\nPrice: ${filtered_df['price'].iloc[i]}\nRating: {filtered_df['review_rating'].iloc[i]}\nNumber of Reviews: {filtered_df['number_of_reviews'].iloc[i]}"
            folium.Marker(
